    ...
    heading_(0)
{
    ...
    velocity_subscriber_ = nh_.subscribe("raw_vel", 50, &AmrBase::velCallback, this);
}

void AmrBase::velCallback(const geometry_msgs::Pose& vel)
{
    ros::Time current_time = ros::Time::now();

    // Assuming 'vel' is of type geometry_msgs::Pose
    linear_velocity_x_ = vel.position.x;
    linear_velocity_y_ = vel.position.y;
    angular_velocity_z_ = vel.orientation.z;
