#include "amr_controller/simple_controller.h"
                                  ...
                                  , theta_(0.0)
{
    ...
    rear_right_cmd_pub_ = nh_.advertise<std_msgs::Float64>("wheel_rear_right_controller/command", 10);
    rear_left_cmd_pub_ = nh_.advertise<std_msgs::Float64>("wheel_rear_left_controller/command", 10);
    front_right_cmd_pub_ = nh_.advertise<std_msgs::Float64>("wheel_front_right_controller/command", 10);
    front_left_cmd_pub_ = nh_.advertise<std_msgs::Float64>("wheel_front_left_controller/command", 10);
    vel_sub_ = nh_.subscribe("amr_controller/cmd_vel", 1000, &SimpleController::velCallback, this);
    joint_sub_ = nh_.subscribe("joint_states", 1000, &SimpleController::jointCallback, this);
    odom_pub_ = nh_.advertise<nav_msgs::Odometry>("amr_controller/odom", 10);
...
void SimpleController::velCallback(const geometry_msgs::Twist &msg)
{
    ...
    rear_right_cmd_pub_.publish(right_speed);
    rear_left_cmd_pub_.publish(left_speed);
    front_right_cmd_pub_.publish(right_speed);
    front_left_cmd_pub_.publish(left_speed);
}
...
void SimpleController::jointCallback(const sensor_msgs::JointState &state)
{
    ...
    theta_ += d_theta;
    x_ += (d_s * cos(theta_)) + (-0.5 * sin(theta_) * d_theta);
    y_ += (d_s * sin(theta_)) + (0.5 * cos(theta_) * d_theta);
